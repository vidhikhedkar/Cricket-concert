...
const LiveMatchCenter = () => {
  ...
  const handleMouseDown = (e) => {
    ...
    scrollRef.current.style.scrollBehavior = 'auto'; // Disable smooth scroll while dragging to prevent lag/acceleration
  };
  ...
  const handleMouseLeaveOrUp = () => {
    ...
    if (scrollRef.current) {
      ...
      scrollRef.current.style.scrollBehavior = 'smooth'; // Re-enable smooth scroll after drag
    }
  };
  ...
  const handleMouseMove = (e) => {
    ...
    // Controlled 1:1 movement ratio (1.0 factor) to eliminate high speed/overshooting
  ...
  return (
    <div className="container py-4 sm:py-14 font-sans select-none overflow-hidden">
      ...
      <div
        ...
        className="-mx-4 px-4 sm:mx-0 sm:px-0 flex gap-3 sm:gap-4 overflow-x-auto pb-4 pt-1 snap-x snap-mandatory scrollbar-none touch-pan-x cursor-grab"
      ...
      <div className="flex justify-center items-center gap-1.5 sm:gap-2 mt-2">
        {matchesData.map((_, idx) => (
          <button
            ...
            className={`transition-all duration-300 cursor-pointer rounded-full h-1.5 sm:h-2 ${
              activeIndex === idx
